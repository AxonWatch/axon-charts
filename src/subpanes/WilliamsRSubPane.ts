...
import { williamsR as computeWilliamsR } from '../utils/indicators.js';

/**
 * Williams %R sub-pane. Oscillator in [-100, 0] range.
 * Overbought at -20, oversold at -80 by default.
 */
export class WilliamsRSubPane extends ScalePane {
  readonly id = 'williamsR';
  readonly label = 'Williams %R';

  getOptions() { return this.chart.options.williamsR || {}; }
  ...
  protected computeValues(chart: IChart): number[] | null {
    ...
    return computeWilliamsR(data, this.getOptions().period ?? 14);
  }
  ...
  renderContent(ctx: CanvasRenderingContext2D, chart: IChart, _subPaneTop: number, _subPaneHeight: number,
    ...
    if (opts.showLevels !== false) {
      ...
      for (const level of [opts.overbought ?? -20, opts.oversold ?? -80]) {
